...
## 4.4 The fidelity experiment, 2026-09-08

Four configurations over a 32-file subset of the sweep corpus, plus a second submission
of the shipped one, all decode-only. Tables in
`tools/quality_sweep/results/2026-09-08-fidelity/fidelity.md`, argument and evidence in
`recommendations.md` beside it, regenerated by `tools/quality_sweep/fidelity.py`.

Read-aloud scripture stretches are scored by word error rate against the World English
Bible, with five other public-domain translations used to confirm the ordering does not
depend on the reference. **Those rates are not accuracy.** The preacher's translation is
unknown and differs from all of them, which inflates every rate; only the difference
between two configurations on the same passage is meaningful. The load-bearing instrument
is not the rate but the run inventory: every one-sided run of twelve or more words between
two decodes, checked against the legacy archive. All twenty such runs are in the legacy
transcript, so each is a real difference in content.

**Three changes were tested and none is adopted. One trigger change is proposed.**

### Sampling instead of beam search on the primary pass: not adopted

Starting the ladder at 0.2 recovers fourteen contiguous runs totalling 578 words that the
shipped decode loses and the legacy archive has, ten of them scripture read aloud,
including the two open cases: Exodus 24 in `tcf.20250607` from 0.834 to 0.442 and
1 Peter 1:22-24 in `ucf20211106b` from 0.676 to 0.437, with the same ordering under all
six translations. Eight of the ten were not previously known to be missing. It costs six
runs totalling 273 words the other way, one of which is a 137-word repetition loop the
sampled pass is right to lack; the remaining 136 words are real, and include Psalm 1:1 at
the opening of `tcf.20230603_mens_fast`, which is the same defect moved to another file.

Not adopted because the evidence is one unseeded draw per file, and the configuration
family contradicts itself inside the dataset: the shipped rescue, which already samples
from 0.2, ran on `tcf.20250607` and was not selected on the very file where the sampled
primary recovered Exodus 24. Because it would make every file irreproducible, where the
shipped configuration returns a byte-identical transcript on 23 of 32 repeat submissions.
Because it moves total segments from 18 325 to 15 449 and individual files by a factor of
four in both directions, and 0.6.0 aligns per segment, and every configuration here is
decode-only so the aligner was never exercised. And because it does not fix
`tcf.20150424`, which stays about 14 percent short either way.

Settling it needs three submissions of the sampled configuration with the aligner
enabled, reporting the run inventory per draw and `agree250`.

### The profile selector keyed on sample and bit rate instead of level: not adopted

On the 20 files whose profile changes, median segments per minute moves 15.16 to 14.66 and
mean segment length 3.48 s to 3.50 s, which is no change; nine files fragment less, six
more, five are unchanged. It costs words: median -0.06 percent, mean -0.43 percent, seven
files losing more than 1 percent, and the subset moves from 981 words under the legacy
archive to 1357 under it. Uncovered speech rises from 2440 s to 2717 s. The one clear
fidelity win, `ucf20211106b` recovering 1 Peter 1:22-24, arrives with that file going from
10.3 to 31.7 segments per minute. Alignment quality is unmeasured, and alignment is what
fragmentation damages.

The earlier finding that level is non-monotonic stands and remains a reason to distrust
the current rule. It is not a reason to adopt this one. If the selector is revisited it
should be as a two-sided level rule, measured with the aligner running.

### The rescue's two levers do not compose

`RESCUE_TEMPERATURE_BASE=0.2` and `condition_on_previous_text=False` are not
interchangeable, and conditioning-off is doing the work. Running the rescue ladder from
0.2 with conditioning left on loses a 91-word run on `tcf.20210217`, taking Matthew 6 from
0.435 to 0.770, and a 70-word run on `tcf.20260626`. On `tcf.20250607` it is identical to
the shipped configuration and Exodus 24 stays lost either way. Leave both settings alone.

Exodus 24 cannot be recovered economically by tuning the rescue: the rescue already runs
on that file with both levers at their best measured values and its output was not
selected. Only a differently sampled draw recovered it, once.

### Proposed for 0.6.1: `RESCUE_MAX_GAP_S`, default 8.0

The detection question is settled against the signal the previous note favoured.
**Uncovered speech fraction does not separate the populations**: bad files run 0.053 to
0.134 and healthy 0.011 to 0.128, the three highest values in the subset are healthy
recordings, and at every false-positive budget it catches one of seven. Coverage ratio is
the same statistic inverted and fails identically. Word rate over speech and flagged
segments catch none at zero false positives.

**The largest contiguous uncovered gap does separate them, at a threshold nobody tried.**
Against the seven files where a second decode and the legacy archive agree that content
was lost, a threshold of 8.5 s reaches all seven and flags one healthy file; at 20 s it
reaches two. The earlier work only evaluated this signal at 10 s and above and only as a
quarantine gate, where a false positive costs a published transcript. As a trigger for a
second decode a false positive costs GPU time, so the threshold belongs far lower.

**Cross-configuration disagreement works only in its directional form.** Symmetric
disagreement between two decodes catches one of seven at zero false positives, because it
also fires when the second decode is the wrong one: the largest reverse run in the corpus,
137 words, is a hallucination loop. The longest run the second decode has that the
published pass lacks is the statistic that works, and it is the only signal that sees
`tcf.20150424` at all. It costs a whole second decode, which is why it belongs behind the
gap filter rather than on every file.

So:

- Add `RESCUE_MAX_GAP_S`, default 8.0. A primary pass whose largest contiguous uncovered
  stretch reaches it fires the rescue, alongside the existing `RESCUE_ANOMALY_WINDOWS`.
- Log and publish the longest run the rescue has that the primary lacks, as a review
  marker.
- Do not quarantine and do not requeue on either signal. Selection already prefers the
  pass with more words, so the decode that detects the loss repairs it.

Measured effect on the subset: the rescue fires on 8 files of 32 rather than 3, and
reaches every confirmed loss instead of two of seven. Cost about 15 to 25 percent more
decode; 11 of the 101 sweep files had a gap of 10 s or more. Risk: those files stop being
reproducible between submissions, which is already true of every file the rescue touches.
Re-validation: rerun the 101-file sweep with the new trigger and confirm no file loses
words to a selected rescue, that the retention floor still blocks the cases it was built
for, and that `agree250` does not move.

The 0.6.1 note on `tcf.20150424` in 4.3 is superseded. It is not undetectable; it is
